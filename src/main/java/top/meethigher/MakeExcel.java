...
//	    font2.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);// 粗体显示
	...
	public void testCascade2007() {
	    // 查询所有的省名称
		List<String> provNameList = makeProvNameList();

	    // 整理数据，放入一个Map中，mapkey存放父地点，value 存放该地点下的子区域
	    Map<String, List<String>> siteMap =  makeSiteMap();
	   ...
	   // row0.createCell(0).setCellValue("<必填>订单类型");
	    ...
	    RichTextString text8 = new XSSFRichTextString("<必填>收货省");
	    text8.applyFont(0, 7, font2); // 0, 1 表示应用字体的范围
	    cell8.setCellValue(text8); // 设置单元格的值
	    sheet1.setColumnWidth(8, 14* 256);

	    //row0.createCell(9).setCellValue("<必填>收货市");
	    Cell cell9 = row0.createCell(9);
	    RichTextString text9 = new XSSFRichTextString("<必填>收货市");
	    text9.applyFont(0, 7, font2); // 0, 1 表示应用字体的范围
	    cell9.setCellValue(text9); // 设置单元格的值
	    sheet1.setColumnWidth(9, 14* 256);
	    //row0.createCell(10).setCellValue("<必填>收货区");

	    Cell cell10 = row0.createCell(10);
	    RichTextString text10 = new XSSFRichTextString("<必填>收货市");
	    ...
	    CellRangeAddressList provRangeAddressList = new CellRangeAddressList(1, 50000, 8, 8);
	    ...
	    // 浙江省下的区域信息。
	    DataValidationConstraint formula = dvHelper.createFormulaListConstraint("INDIRECT($I$" + 2 + ")");
	    CellRangeAddressList rangeAddressList = new CellRangeAddressList(1,50000,9,9);
	    ...
	    // 区规则
	    formula = dvHelper.createFormulaListConstraint("INDIRECT($J$" + 2 + ")");
	    rangeAddressList = new CellRangeAddressList(1,50000,10,10);
	    cacse = dvHelper.createValidation(formula, rangeAddressList);
	    cacse.createErrorBox("error", "请选择正确的区");
	    cacse.setEmptyCellAllowed(true);
	    cacse.setShowErrorBox(true);
	    cacse.setSuppressDropDownArrow(true);
	    sheet1.addValidationData(cacse);
	  ...
	  //创建一个专门用来存放地区信息的隐藏sheet页
	    ...
	    // 查询所有的订单类型名称
		List<String> orderTypeList = makeOrderType();

	    // 整理数据，放入一个Map中，mapkey存放父订单类型，value 存放该地点下的订单服务类型
	    Map<String, List<String>> makeOrderServiceMap =  makeOrderServiceMap();

	    int rowId1 = 0;
	    // 设置第一行，存省的信息
	    Row proviRow1 = hideSheet2.createRow(rowId1++);
	    proviRow1.createCell(0).setCellValue("订单类型");
	    for(int j = 0; j < orderTypeList.size(); j ++){
	        Cell proviCell = proviRow1.createCell(j + 1);
	        proviCell.setCellValue(orderTypeList.get(j));
	    }
	    // 将具体的数据写入到每一行中，行开头为父级区域，后面是子区域。
	    Iterator<String> keyIterator1 = makeOrderServiceMap.keySet().iterator();
	    while(keyIterator1.hasNext()){
	        String key = keyIterator1.next();
	        List<String> son = makeOrderServiceMap.get(key);

	        Row row1 = hideSheet2.createRow(rowId1++);
	        row1.createCell(0).setCellValue(key);
	        for(int j = 0; j < son.size(); j ++){
	            Cell cell21 = row1.createCell(j + 1);
	            cell21.setCellValue(son.get(j));
	        }

	        // 添加名称管理器
	        String range = getRange(1, rowId1, son.size());
	        Name name = book.createName();
	        name.setNameName(key);
	        String formula1 = "site2!" + range;
	    ...
	    // 订单类型规则
	    DataValidationConstraint provConstraint1 = dvHelper1.createExplicitListConstraint(orderTypeList.toArray(new String[]{}));
	    CellRangeAddressList provRangeAddressList1 = new CellRangeAddressList(1, 50000, 0, 0);
	    DataValidation provinceDataValidation1 = dvHelper1.createValidation(provConstraint1, provRangeAddressList1);
	    provinceDataValidation1.createErrorBox("error", "请选择正确的订单类型");
	    ...
	    // 订单服务规则，此处仅作一个示例
	    // "INDIRECT($A$" + 2 + ")" 表示规则数据会从名称管理器中获取key与单元格 A2 值相同的数据，如果A2是安装单，那么此处就是
	    // 安装单下的服务信息。
	    DataValidationConstraint formula1 = dvHelper1.createFormulaListConstraint("INDIRECT($A$" + 2 + ")");
	    CellRangeAddressList rangeAddressList1 = new CellRangeAddressList(1,50000,1,1);
	    DataValidation cacse1 = dvHelper1.createValidation(formula1, rangeAddressList1);
	    cacse1.createErrorBox("error", "请选择正确的订单服务类型");
	    cacse1.setShowErrorBox(true);
	    cacse1.setSuppressDropDownArrow(true);
	    sheet1.addValidationData(cacse1);
	 ...
	 * 制作所有的省的list
	 */
	public List<String>  makeProvNameList (){
		List<String> provNameList = new ArrayList<String>();
		try {
			JSONArray jsonArray = new JSONArray().parseArray(provinceList);
	 ...
	 * @return
	 */
	public Map<String, List<String>> makeSiteMap(){
		...
		try {
			...
			for(int i = 0;i<jsonArray.size();i++){
				...
				JSONArray jsonArrays = new JSONArray().parseArray(jsonob.getString("cityList"));
				...
				for(int j = 0;j<jsonArrays.size();j++){
					...
					JSONArray jsonArrayse = new JSONArray().parseArray(jsonobs.getString("areaList"));

					List<String> liste = new ArrayList<String>();
					for(int k = 0;k<jsonArrayse.size();k++){
						liste.add(jsonArrayse.get(k).toString());
					}
	 ...
	 * @return list
	 */
	public List<String>  makeOrderType(){
		List<String> list = new ArrayList<String>();
		try {
			list.add("安装单");
			list.add("测量单");
	 ...
	 * @return
	 */
	public Map<String, List<String>> makeOrderServiceMap(){
